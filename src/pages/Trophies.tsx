...
const Trophies = () => {
  ...
  const loadLeaderboard = async () => {
    try {
      setIsLoading(true);
      const data = await leaderboardApi.getTrophiesLeaderboard(100);
      setPlayers(data);
    } catch (error) {
      console.error("Failed to load trophies leaderboard:", error);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="space-y-8">
        ...
        <div className="text-center space-y-4">
          <div className="flex items-center justify-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-bold">Trophies Leaderboard</h1>
          </div>
          <p className="text-muted-foreground">Ranked by number of trophies earned</p>
          ...
          <div className="grid grid-cols-3 gap-4 max-w-4xl mx-auto">
            ...
            <Card className="p-6 bg-gradient-to-br from-gray-400/10 to-card border-gray-400/30 text-center mt-8">
              ...
              <Badge className="bg-yellow-500/20 text-yellow-500 border-yellow-500/30">
                <Trophy className="w-3 h-3 mr-1 inline" />{filteredPlayers[1].trophy_count || 0}
              </Badge>
            </Card>
            ...
            <Card className="p-6 bg-gradient-to-br from-yellow-500/20 to-card border-yellow-500/50 text-center shadow-gold">
              ...
              <Badge className="bg-yellow-500/20 text-yellow-500 border-yellow-500/30 text-lg">
                <Trophy className="w-4 h-4 mr-1 inline" />{filteredPlayers[0].trophy_count || 0}
              </Badge>
            </Card>
            ...
            <Card className="p-6 bg-gradient-to-br from-orange-600/10 to-card border-orange-600/30 text-center mt-8">
              ...
              <Badge className="bg-orange-600/20 text-orange-600 border-orange-600/30">
                <Trophy className="w-3 h-3 mr-1 inline" />{filteredPlayers[2].trophy_count || 0}
              </Badge>
            </Card>
          </div>
        )}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <span className="font-bold text-lg text-yellow-500">{player.trophy_count || 0}</span>
                      </div>
                    </td>
                  </tr>
                ))}
